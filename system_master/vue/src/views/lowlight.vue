<template>
	...
	<el-dialog title="微光影像导入" v-model="isVisible" width="850px" @close="handleDialogClose">
		<div class="dialog">
		<el-upload
			ref="uploadRef"
			class="upload-demo"
			action="http://localhost:8080/lowlight/uploadImages"
			:auto-upload="false" 
...
const getData = () => {
	...
	if(route.params.mmsi == undefined) {
		axios.get('http://localhost:8080/lowlight/getAllInfo').then((res) => {
		if(res.status == 200) {
		...
		axios.post('http://localhost:8080/lowlight/getInfo', formData).then((res) => {
		if(res.status == 200) {
...
async function downloadImages() {
  ...
  zip.generateAsync({ type: 'blob' }).then((content) => {
    const zipName = 'lowlight_image_folder.zip';
